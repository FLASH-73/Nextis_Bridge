...
class RealSenseCamera(Camera):
    ...
    def _read_color_and_depth(
        self, color_mode: ColorMode | None = None, timeout_ms: int = 200
    ) -> tuple[NDArray[Any], NDArray[Any] | None]:
        """
        Reads both color and depth frames from the same frameset.

        This is used by the background thread to capture synchronized frames.

        Args:
            color_mode (Optional[ColorMode]): The target color mode (RGB or BGR).
            timeout_ms (int): Maximum time in milliseconds to wait for frames.

        Returns:
            Tuple of (color_image, depth_image). depth_image is None if depth is not enabled.
        """
        if not self.is_connected:
            raise DeviceNotConnectedError(f"{self} is not connected.")
    ...
    def _read_loop(self) -> None:
        ...
        On each iteration:
        1. Reads color (and depth if enabled) frames with 500ms timeout
        2. Stores result in latest_frame and latest_depth_frame (thread-safe)
        ...
        while not self.stop_event.is_set():
            try:
                # Read both color and depth from the same frame set
                color_image, depth_image = self._read_color_and_depth(timeout_ms=500)
    ...
    def async_read(self, timeout_ms: float = 200, blocking: bool = True) -> NDArray[Any]:
        ...
        Args:
            ...
            blocking (bool): If True, waits for a new frame event. 
        ...
        # ZOH: If non-blocking and we have a frame, return it immediately
        if not blocking:
             with self.frame_lock:
                 if self.latest_frame is not None:
                     return self.latest_frame
                 # If no frame is available yet (e.g. startup), we must block to avoid returning None
    ...
    def async_read_depth(self, timeout_ms: float = 200, blocking: bool = True) -> NDArray[Any]:
        """
        ...
        if not self.use_depth:
            raise RuntimeError(f"Depth stream is not enabled for {self}. Set use_depth=True in config.")
        ...
        if frame is None:
            raise RuntimeError(f"No depth frame available for {self}. Ensure depth stream is enabled.")
