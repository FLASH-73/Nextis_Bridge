...
class OpenCVCamera(Camera):
    ...
    def __init__(self, config: OpenCVCameraConfig):
        ...
        self.frame_timestamp: float = 0.0  # Track when frame was captured
    ...
    def connect(self, warmup: bool = True) -> None:
        ...
        # Minimize buffer to reduce latency (default is 4 frames = 133ms lag at 30fps)
        # Setting to 1 ensures we always get the most recent frame
        self.videocapture.set(cv2.CAP_PROP_BUFFERSIZE, 1)

        # Enable auto-exposure for V4L2 cameras (Linux)
        # V4L2 values: 1 = Manual, 3 = Auto (NOT 0.75 which was wrong!)
        if platform.system() == "Linux":
            try:
                self.videocapture.set(cv2.CAP_PROP_AUTO_EXPOSURE, 3)  # 3 = auto mode for V4L2
                logger.debug(f"{self} auto-exposure set to auto mode (V4L2)")
            except Exception as e:
                logger.debug(f"{self} failed to set auto-exposure: {e}")

        if warmup:
            start_time = time.time()
            while time.time() - start_time < self.warmup_s:
                try:
                    frame = self._read_sync()
                    # Store last warmup frame so latest_frame is never None
                    with self.frame_lock:
                        self.latest_frame = frame
                        self.frame_timestamp = time.perf_counter()
                except Exception as e:
                    # Keep trying during warmup - USB might be congested by other cameras
                    logger.debug(f"{self} warmup read failed, retrying: {e}")
                time.sleep(0.1)

        # Pre-start background thread so async_read has frames ready immediately
    ...
    def read(self, color_mode: ColorMode | None = None) -> NDArray[Any]:
        ...
        """
        return self._read_sync(color_mode)

    def _read_sync(self, color_mode: ColorMode | None = None) -> NDArray[Any]:
        """
        Reads a single frame synchronously from the camera.
    ...
    def _read_loop(self) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if not ret:
                    # Capture failed - add delay to prevent tight spin loop
                    # This gives USB bus breathing room when congested
                ...
                import traceback
                traceback.print_exc()
                logger.error(f"Camera Read Loop Failed for {self}: {e}")
    ...
    def async_read(self, timeout_ms: float = 200, blocking: bool = True) -> NDArray[Any]:
        """
        ...
        Args:
            ...
            blocking (bool): If True, waits for a NEW frame (since last read).
                If False, returns the latest available frame immediately (Zero-Order Hold).
                If False and no frame is available yet, it will wait up to timeout.
        ...
        """
        # For non-blocking (ZOH) reads, skip connection check - return cached frame regardless
        # This makes video feed resilient to temporary USB hiccups
        if blocking and not self.is_connected:
            raise DeviceNotConnectedError(f"{self} is not connected.")

        if self.thread is None or not self.thread.is_alive():
            self._start_read_thread()

        # Zero-Order Hold: When blocking=False, return cached frame immediately (even if stale/None)
        # This prevents multi-second delays when USB camera has issues
        # A stale frame is better than freezing the video feed or showing "Waiting..."
        if not blocking:
            with self.frame_lock:
                return self.latest_frame  # Return whatever we have (might be None, might be stale)
        ...
        # Clear event INSIDE the lock to prevent race condition where
        # a new frame arrives between getting the frame and clearing
        with self.frame_lock:
