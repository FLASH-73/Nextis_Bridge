...
def load_image_as_numpy(
    ...
    # Check if this is a depth image (by path pattern or image mode)
    is_depth = "_depth" in fpath_str or img.mode in ("I", "I;16", "L")

    if is_depth:
        # For depth images, keep as single channel
        # Handle 16-bit depth images (mode "I;16" or "I")
        ...
        if channel_first:  # (H, W, 1) -> (1, H, W)
            img_array = np.transpose(img_array, (2, 0, 1))

        # Convert to target dtype
        if np.issubdtype(dtype, np.floating):
            # Normalize: uint16 depth typically in mm, scale to meters (0-65.535m range)
            if img_array.max() > 255:
                img_array = img_array.astype(dtype) / 65535.0
            else:
    ...
    # For RGB images, convert and process as before
    img = img.convert("RGB")
...
def hw_to_dataset_features(
    ...
    for key, shape in cam_fts.items():
        # Detect depth features by name or shape (single channel)
        ...
        # Add depth metadata for video features
        if use_video and is_depth:
            feature_dict["info"] = {"video.is_depth_map": True}
        features[f"{prefix}.images.{key}"] = feature_dict
