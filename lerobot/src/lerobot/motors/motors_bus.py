...
class MotorsBus(abc.ABC):
    ...
    def disconnect(self, disable_torque: bool = True) -> None:
        ...
        if not self.is_connected:
            logger.info(f"{self.__class__.__name__}('{self.port}') already disconnected.")
            return
        ...
        if disable_torque:
            try:
                ...
                logger.warning(f"Failed to disable torque during disconnect (ignoring): {e}")
    ...
    def _unnormalize(self, ids_values: dict[int, float]) -> dict[int, int]:
        ...
        for id_, val in ids_values.items():
            ...
            max_ = self.calibration[motor].range_max
            drive_mode = self.apply_drive_mode and self.calibration[motor].drive_mode
            # logger.error(f"DEBUG: Calibration Check for {motor}: min={min_}, max={max_}")
            if max_ == min_:
                logger.error(f"DEBUG: Calibration Error (Unnormalize) for {motor}. min={min_}, max={max_}")
                raise ValueError(f"Invalid calibration for motor '{motor}': min and max are equal.")
