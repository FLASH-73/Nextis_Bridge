...
export default function RecordingModal({ isOpen, onClose, maximizedWindow, setMaximizedWindow }: RecordingModalProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            fetch(`${API_BASE}/recording/options`)
                .then(res => res.json())
                .then(data => {
    ...
    const startSession = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/recording/session/start`, {
                method: 'POST',
            ...
            if (data.status === 'success') {
                ...
                const teleopRes = await fetch(`${API_BASE}/teleop/start`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ force: false })
                });
                const teleopData = await teleopRes.json();
    ...
    const stopSession = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE}/recording/session/stop`, { method: 'POST' });
            const data = await res.json();
    ...
    const toggleEpisode = async () => {
        ...
        try {
            if (isRecordingEpisode) {
                ...
                const res = await fetch(`${API_BASE}/recording/episode/stop`, { method: 'POST' });
                const data = await res.json();
                ...
                const res = await fetch(`${API_BASE}/recording/episode/start`, { method: 'POST' });
                const data = await res.json();
                ...
                if (data.status === 'success') {
                    ...
                    setError(data.message || 'Failed to start episode');
                }
            }
        } catch (e: any) {
                                            ...
                                            <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-zinc-900 border border-neutral-200 dark:border-zinc-700 rounded-xl shadow-sm">
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        ...
                                                        try {
                                                            await fetch(`${API_BASE}/recording/episode/last`, { method: 'DELETE' });
